...
- ### Veritabanı Etkinleştirme
  ...
  DbScriptAndInfos klasörünün altında bulunan `script.sql` içinde `7.` ve `9.` satırda bulunan dosya yolunu kendi bilgisayarınıza göre düzenlemeniz gerekmektedir. Bu işlemden sonra `script.sql` dosyası mevcut veritabanı üzerinde çalıştırılmalıdır (SQLServer).
